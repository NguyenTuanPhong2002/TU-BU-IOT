...
void RS485_Master_Receive(char Master_ReceivedBuff[], int length) {
	...
	for (int s = 0; s < length; s++) {
		...
		if (UART6_ReceivedBuff[index_Adr] == 0x01
				&& UART6_ReceivedBuff[index_Func] == 0x03) {
			for (int v = 0; v < length; v++) {
				Master_ReceivedBuff[v] = UART6_ReceivedBuff[(v + index_Adr)
															% length];
			}
		}
		if (UART6_ReceivedBuff[index_Adr] == 0x01
				&& UART6_ReceivedBuff[index_Func] == 0x01) {
			for (int v = 0; v < length; v++) {
				Master_ReceivedBuff[v] = UART6_ReceivedBuff[(v + index_Adr)
															% length];
			}
