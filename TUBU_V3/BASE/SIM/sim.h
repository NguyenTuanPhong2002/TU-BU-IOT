...
protected:
    ...
    virtual APP_StatusTypeDef SIM_sendATCommand(const char *command, const char *expect, const char *unexpect, uint32_t timeout);

public:
    SIM();
    ~SIM();
    APP_StatusTypeDef getSimData(uint32_t data, SIM_DataTypeDef *pData);
    virtual void *simWakeup();
    virtual void *simSleep();
    bool SIM_Init(SIM_ptr *me);

    virtual int8_t SIM_getRSSI();
    virtual uint8_t SIM_getNwtype(char pNwtype[], uint8_t size);
    virtual APP_StatusTypeDef SIM_getSimPhone(char pPhone[], uint8_t size);
    virtual APP_StatusTypeDef SIM_getIMEI(char pIMEI[], uint8_t size);

    virtual APP_StatusTypeDef SIM_callUSSD(const char *ussd, char response[], uint16_t size);
};
